...
#
# The Metal3 and Cluster API types MetalCluster emits come from their operators,
# not from this project; the apiDependencies in upbound.yaml only generate
# build-time schemas. Without these installed the XRD establishes and the
# composition then fails on types that do not exist.
#
...
# interruption. Default location needs no root. A RHEL-family image by default,
# because the GPU driver install the composition runs uses dnf.
NODE_IMAGE_URL=${NODE_IMAGE_URL:-https://artifactory.nordix.org/artifactory/metal3/images/k8s_v1.36.2/CENTOS_10_NODE_IMAGE_K8S_v1.36.2.qcow2}
...
spec:
  ...
  extraConfig:
    ...
    - group: conductor
      ...
      value: "900"
    # This machine reports PowerState Off for the first few minutes of POST
    # while it is plainly drawing power, so a power-on looks like it failed.
    # Ironic gives up after power_state_change_timeout and the operation is
    # retried into a host that is already booting, which resets it, and it
    # never finishes POST. Must exceed the real POST time, not the BMC's idea
    # of it.
    - group: conductor
      name: power_state_change_timeout
      value: "600"
    # Only record what the BMC reports, never push the host back to the value
    # in the database. Forcing turns a slow boot into a fight.
    - group: conductor
      name: force_power_state_during_sync
      value: "false"
    - group: conductor
...
# IrSO does not roll the Ironic deployment when extraConfig changes, and the
# settings above reach Ironic as environment variables rather than through
# ironic.conf. Changing them on an existing install needs
# `kubectl -n $IRONIC_NS rollout restart deploy/ironic-service`, which also
# empties Ironic's node database; BMO re-enrolls from the BareMetalHosts.
...
RBAC="$(cd "$(dirname "$0")/.." && pwd)/examples/rbac.yaml"
if [ -f "$RBAC" ]; then
  say "Crossplane RBAC"
  k apply -f "$RBAC"
fi
